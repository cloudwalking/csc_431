...
main:
 ...
 !#function-body:
	...
	mov	 %l1, %l1	 
	!LOADI	0, 85
	set	 0, %l1	 	!int: reg 85 gets val 0
	!MOV	80, 86
	mov	 %l3, %l2	 
	!STOREAI	85, 86, 0
	st	%l1, [%l2 + 0]
...
.S30:
 ...
 .S32:
	...
	mov	 %l1, %o0	 	!argument: store out param from reg 189to register outReg 1
	!CALL	countTotal
	call	 countTotal	
	nop
	!MOV	1, 188
	...
	nop
	!LOADI	1, 191
	set	 1, %l1	 	!int: reg 191 gets val 1
	!STOREOUTARGUMENT	191, 5
	mov	 %l1, %o0	 	!argument: store out param from reg 191to register outReg 1
	!CALL	convertBinary
	call	 convertBinary	
	nop
	!MOV	1, 190
	...
	nop
	!LOADI	2, 193
	set	 2, %l1	 	!int: reg 193 gets val 2
	!STOREOUTARGUMENT	193, 5
	mov	 %l1, %o0	 	!argument: store out param from reg 193to register outReg 1
	!CALL	convertBinary
	call	 convertBinary	
	nop
	!MOV	1, 192
	...
	nop
	!LOADI	3, 195
	set	 3, %l1	 	!int: reg 195 gets val 3
	!STOREOUTARGUMENT	195, 5
	mov	 %l1, %o0	 	!argument: store out param from reg 195to register outReg 1
	!CALL	convertBinary
	call	 convertBinary	
	nop
	!MOV	1, 194
	...
	nop
	!LOADI	4, 197
	set	 4, %l1	 	!int: reg 197 gets val 4
	!STOREOUTARGUMENT	197, 5
	mov	 %l1, %o0	 	!argument: store out param from reg 197to register outReg 1
	!CALL	convertBinary
	call	 convertBinary	
	nop
	!MOV	1, 196
	...
	nop
	!LOADI	5, 199
	set	 5, %l1	 	!int: reg 199 gets val 5
	...
	nop
	!LOADI	6, 201
	set	 6, %l1	 	!int: reg 201 gets val 6
	...
	nop
	!LOADI	7, 203
	set	 7, %l1	 	!int: reg 203 gets val 7
	...
	nop
	!LOADI	8, 205
	set	 8, %l1	 	!int: reg 205 gets val 8
	...
	nop
	!LOADI	9, 207
	set	 9, %l1	 	!int: reg 207 gets val 9
	...
	mov	 %l1, %o1	 	!argument: store out param from reg 206to register outReg 2
	!CALL	printRecursive
	call	 printRecursive	
	nop
	!LOADI	0, 4
 ...
 .S36:
	!LOADI	1, 130
	set	 1, %l1	 	!int: reg 130 gets val 1
	!MOV	80, 129
	mov	 %l3, %l2	 
	!LOADAI	129, 4, 129
	ld	[%l2 + 4], %l2
 ...
 .S39:
	!LOADI	1, 138
	set	 1, %l1	 	!int: reg 138 gets val 1
	!MOV	80, 137
	mov	 %l3, %l2	 
	!LOADAI	137, 8, 137
	ld	[%l2 + 8], %l2
 ...
 .S42:
	!LOADI	1, 146
	set	 1, %l1	 	!int: reg 146 gets val 1
	!MOV	80, 145
	mov	 %l3, %l2	 
	!LOADAI	145, 12, 145
	ld	[%l2 + 12], %l2
 ...
 .S45:
	!LOADI	1, 154
	set	 1, %l1	 	!int: reg 154 gets val 1
	!MOV	80, 153
	mov	 %l3, %l2	 
	!LOADAI	153, 16, 153
	ld	[%l2 + 16], %l2
 ...
 .S48:
	!LOADI	1, 162
	set	 1, %l1	 	!int: reg 162 gets val 1
	!MOV	80, 161
	mov	 %l3, %l2	 
	!LOADAI	161, 20, 161
	ld	[%l2 + 20], %l2
 ...
 .S51:
	!LOADI	1, 170
	set	 1, %l1	 	!int: reg 170 gets val 1
	!MOV	80, 169
	mov	 %l3, %l2	 
	!LOADAI	169, 24, 169
	ld	[%l2 + 24], %l2
 ...
 !#if-guard:
	...
	set	 1, %g1	 
	!COMP	173, 3, 2
	cmp	 %l4, %g1	 	!if: compare guard result (reg 173) to true
	!CBREQ	2, .S54, .S55
 ...
 .S54:
	!LOADI	1, 178
	set	 1, %l1	 	!int: reg 178 gets val 1
	!MOV	80, 177
	mov	 %l3, %l2	 
	!LOADAI	177, 28, 177
	ld	[%l2 + 28], %l2
 ...
 .S55:
	!LOADI	1, 183
	set	 1, %l1	 	!int: reg 183 gets val 1
	!MOV	80, 182
	mov	 %l3, %l2	 
	!LOADAI	182, 32, 182
	ld	[%l2 + 32], %l2
